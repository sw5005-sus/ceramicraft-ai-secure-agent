...
def update_user_register_time(useractivatedEvent: UserActivatedEvent) -> None:
    ...
    try:
        redis_client.set(f"u:{user_id}:rt", activated_time)
        logger.info(
...
def get_user_register_time(user_id: int) -> int:
    ...
    try:
        activated_time = redis_client.get(f"u:{user_id}:rt")
...
def update_user_ip(user_id: int, ip_address: str) -> None:
    ...
    try:
        redis_client.zset(
            f"u:{user_id}:ip", ip_address, int(datetime.now().timestamp())
        )
        redis_client.zremrangebyscore(f"u:{user_id}:ip", 0, latest_retain_time)
        logger.info(f"User {user_id} IP address {ip_address} stored in Redis.")
    except Exception as e:
